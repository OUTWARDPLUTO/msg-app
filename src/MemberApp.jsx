// MemberApp.jsx — Member-facing fitness UI (extracted from original App.jsx)
// All original components preserved; gymId + engagement tracking added.
...
export default function MemberApp({
  user, gymId, gymName, darkMode, onToggleTheme, onLogout,
  ...
  const handleSaveProgress = (entry) => {
    ...
    import('./shared/firebase.js').then(m => m.trackActivity(user?.uid, gymId, 'progress'));
  };

  const handleWorkoutComplete = () =>
    import('./shared/firebase.js').then(m => m.trackActivity(user?.uid, gymId, 'workout'));

  const handleMealLogged = () =>
    import('./shared/firebase.js').then(m => m.trackActivity(user?.uid, gymId, 'diet'));

  const views = {
    home: (
      <div>
        <AttendanceButton uid={user?.uid} gymId={gymId} />
        <HomeSection mealLog={mealLog} progressLogs={progressLogs} dietGoal={dietGoal} onLogClick={() => setShowLogModal(true)} user={user} />
      </div>
    ),
    ...
    <div className="msg-root" style={{ position: 'relative', background: C.bg, color: C.text, fontFamily: "'Plus Jakarta Sans',sans-serif", display: 'flex', flexDirection: 'column', height: '100dvh', maxWidth: 430, margin: '0 auto', overflow: 'hidden', colorScheme: darkMode ? 'dark' : 'light' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px 0', flexShrink: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1 }}>
          ...
          <div style={{ fontFamily: fn, fontSize: 8.5, fontWeight: 600, letterSpacing: '0.12em', color: C.muted, textTransform: 'uppercase', marginTop: 1 }}>
            {gymName || 'My Smart Gains'}
          </div>
        </div>
        <button onClick={() => setShowProfile(p => !p)} style={{ width: 36, height: 36, borderRadius: '50%', background: C.accent, border: `2px solid ${showProfile ? C.text : 'transparent'}`, cursor: 'pointer', fontFamily: fn, fontSize: 11, fontWeight: 800, color: '#111', transition: 'all 0.2s', boxShadow: C.accentShadow, overflow: 'hidden', padding: 0 }}>
          {user?.photo ? <img src={user.photo} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (user?.name || '?').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()}
        </button>
      </div>
