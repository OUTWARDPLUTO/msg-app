...
export default function TrainerInbox({ gymId, user, setBackHandler }) {
  ...
  // Listen to all chats for this trainer in the gym
  useEffect(() => {
  ...
  if (openChat) {
    ...
    return (
      <ChatScreen
        chatId={chatData.id}
        chatMeta={{ gymId, memberUid: chatData.memberUid, trainerUid: user.uid, type: 'trainer', otherName: memberInfo?.name || 'Member' }}
        senderUid={user.uid}
        senderRole="trainer"
  ...
  // Use unreadOwner since trainers use the "owner" unread field in member-initiated chats
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6, maxWidth: 260, margin: '0 auto' }}>
            When your members message you, their conversations will appear here.
          </div>
        </div>
      ) : (
