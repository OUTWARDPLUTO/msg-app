...
const THEMES = {
  dark: {
    bg: '#111111', s1: '#1A1A1A', s2: '#2F2F2F', s3: '#3A3A3A', s4: '#444444',
...
// C is set dynamically — components read from this object
let C = { ...THEMES.dark };
const fn = "'Plus Jakarta Sans', sans-serif";
const fb = "'Plus Jakarta Sans', sans-serif";
const MC = { chest: '#4E9FFF', back: '#2DD4BF', shoulders: '#A78BFA', arms: '#FF6240', core: '#D99A2B', legs: '#FF6B6B' };
...
// ─── Home Section ────────────────────────────────────────────────────────────
function HomeSection({ mealLog, progressLogs, dietGoal, onLogClick, user }) {
...
// ─── Profile Screen ───────────────────────────────────────────────────────────
function ProfileScreen({ onClose, progressLogs, dietGoal, user }) {
...
// ─── Firebase Auth ─────────────────────────────────────────────────────────
// NOTE: Firebase Auth requires a real browser environment (not an iframe).
// In production (Vercel/deployed app), this works fully.
...
async function getFBAuth() {
  ...
  if (_fbLoading) {
    return new Promise((res, rej) => _fbCallbacks.push({ res, rej }));
  }

  _fbLoading = true;
  try {
    // Load sequentially — auth-compat depends on app-compat
    await loadScript('https://www.gstatic.com/firebasejs/10.12.0/firebase-app-compat.js');
    // Poll for window.firebase to be available
    await new Promise((res, rej) => {
      let attempts = 0;
      const t = setInterval(() => {
        if (window.firebase?.app) { clearInterval(t); res(); }
        if (++attempts > 50) { clearInterval(t); rej(new Error('firebase-app timeout')); }
      }, 100);
    });
    ...
    await new Promise((res, rej) => {
      let attempts = 0;
      const t = setInterval(() => {
        if (window.firebase?.auth) { clearInterval(t); res(); }
        if (++attempts > 50) { clearInterval(t); rej(new Error('firebase-auth timeout')); }
      }, 100);
    });

    if (!window.firebase.apps?.length) window.firebase.initializeApp(FB_CONFIG);
    _fbAuth = window.firebase.auth();
    _fbCallbacks.forEach(cb => cb.res(_fbAuth));
    return _fbAuth;
  } catch (err) {
    _fbCallbacks.forEach(cb => cb.rej(err));
    _fbLoading = false;
    _fbAuth = null;
    throw err;
  }
...
function LoginScreen({ onLogin }) {
  ...
  const handleEmail = async () => {
    ...
    if (fbStatus !== 'ready') { setError('Firebase not loaded. Check your internet connection.'); return; }
  ...
  const handleGoogle = async () => {
    if (fbStatus !== 'ready') { setError('Firebase not loaded yet. Please wait a moment.'); return; }
    setLoading(true); setError('');
    try {
      const auth = await getFBAuth();
  ...
  return (
    <div style={{ position: 'relative', background: C.bg, color: C.text, fontFamily: fn, display: 'flex', flexDirection: 'column', height: '100dvh', maxWidth: 430, margin: '0 auto', alignItems: 'center', justifyContent: 'center', padding: '0 24px', boxSizing: 'border-box', overflowY: 'auto' }}>
      {/* Logo */}
      ...
      <div style={{ width: '100%', background: C.s2, border: `1px solid ${C.border}`, borderRadius: 24, padding: '22px 20px', boxShadow: C.elevShadow }}>
        ...
        <button onClick={handleGoogle} disabled={loading || fbStatus !== 'ready'} style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
          boxShadow: C.cardShadow, opacity: fbStatus !== 'ready' ? 0.5 : 1, transition: 'all 0.2s',
        }}>
      ...
      {/* Demo bypass — for testing without Firebase */}
...
export default function MSG() {
  ...
  // Sync C object whenever theme changes — components re-render through state
  ...
  const [user, setUser] = useState(() => load('msg_user', null)); // {uid,name,email,photo}
  ...
  const setMealLog = fn => setMealLogRaw(prev => { const n = typeof fn === 'function' ? fn(prev) : fn; save('msg_meal_log', n); return n; });
  const setWeekPlan = v => { setWeekPlanRaw(v); save('msg_week_plan', v); };
  const setProgressLogs = fn => setProgressLogsRaw(prev => { const n = typeof fn === 'function' ? fn(prev) : fn; save('msg_progress', n); return n; });
    ...
    if (!document.getElementById('msg-global-css')) {
      ...
      style.textContent = `
        ...
        * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; }
        ...
        /* Smooth fade for theme transitions */
  ...
  useEffect(() => {
    getFBAuth().then(auth => {
      auth.onAuthStateChanged(fbUser => {
        if (fbUser && !user) {
          const u = { uid: fbUser.uid, name: fbUser.displayName || fbUser.email.split('@')[0], email: fbUser.email, photo: fbUser.photoURL };
          setUser(u); save('msg_user', u);
        }
      });
    }).catch(() => { });
  ...
  if (showProfileSetup) return <ProfileSetupScreen user={user} onComplete={u => { setUser(u); save('msg_user', u); setShowProfileSetup(false); }} />;

  const views = {
    home: <HomeSection mealLog={mealLog} progressLogs={progressLogs} dietGoal={dietGoal} onLogClick={() => setShowLogModal(true)} user={user} />,
  ...
  return (
    <div className="msg-root" style={{ position: 'relative', background: C.bg, color: C.text, fontFamily: "'Plus Jakarta Sans',sans-serif", display: 'flex', flexDirection: 'column', height: '100dvh', maxWidth: 430, margin: '0 auto', overflow: 'hidden', colorScheme: darkMode ? 'dark' : 'light' }}>
      ...
      {profileScreen === 'profile' && <ProfileScreen onClose={() => setProfileScreen(null)} progressLogs={progressLogs} dietGoal={dietGoal} user={user} />}
      {profileScreen === 'settings' && <SettingsScreen onClose={() => setProfileScreen(null)} onResetDiet={() => setDietGoal(null)} onResetWorkout={() => setWeekPlan(null)} darkMode={darkMode} onToggleTheme={toggleTheme} />}
